...
import boto3
from utilities import group2head, get_single_image_embeddings, get_only_student_embedding, get_all_student_models
    ...
    try:
        ...
        if len(students) == 1:
            ...
            for idx, person in enumerate(segmented_heads): #list of PIL images
                ...
                sim = F.cosine_similarity(emb, registered_emb.unsqueeze(0)).item()
                if sim > 0.8:
            ...
            for idx, person in enumerate(segmented_heads):  # PIL images
                ...
                for student_name, model in models:
                    with torch.no_grad():
                        ...
                        if prob > 0.8:
                            print(f"Detected {student_name} with confidence {prob:.2f} for head {idx}")
                            x1, y1, x2, y2 = map(int, boxes[idx])
                            cv2.rectangle(img_np, (x1, y1), (x2, y2), (0, 255, 0), 2)
                            cv2.putText(img_np, f"{student_name} ({prob:.2f})", (x1, y1 - 10),
                                        cv2.FONT_HERSHEY_SIMPLEX, 0.6, (0, 255, 0), 2)
