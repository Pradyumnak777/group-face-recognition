...
from api_calls_and_functions.shared_material.util_functions.models_n_training import bin_classifier
...
def group2head_reg(image_data: bytes) -> bytes:
    try:
        ...
        segmentation_map = segmentation_pipeline.predict(img_np)

        segmented_region = img_np * cv2.cvtColor(segmentation_map, cv2.COLOR_GRAY2RGB)
        segmented_region_rgb = cv2.cvtColor(segmented_region, cv2.COLOR_BGR2RGB)
        pil_image = Image.fromarray(segmented_region_rgb)
        segmented_heads.append(pil_image)
